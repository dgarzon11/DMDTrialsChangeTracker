...
def download_studies(page_size):
    ...
    base_url = "https://clinicaltrials.gov/api/v2/studies?query.cond=dmd"
    params = {
        "format": "json",
        "pageSize": page_size,
    }
        ...
        for nct_id, group in df.groupby('NCTId'):
            if len(group) > 1:
                ...
                for i in range(1, len(group)):
                    ...
                    # Columns derived from other columns — skip to avoid duplicate changes
                    DERIVED_COLUMNS = {'MinimumAgeMonths', 'MaximumAgeMonths'}

                    for column in group.columns:
                        if column not in ['NCTId', 'Timestamp'] and column not in DERIVED_COLUMNS:
                            current_value = current_row[column]
                            previous_value = previous_row[column]

                            # Handle numeric values (round to 4dp to avoid float precision false positives)
                            if pd.api.types.is_numeric_dtype(group[column]):
                                current_value = round(float(current_value), 4) if pd.notnull(current_value) else current_value
                                previous_value = round(float(previous_value), 4) if pd.notnull(previous_value) else previous_value
