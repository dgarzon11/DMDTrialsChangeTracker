...
export default function TrialsModal({ studies, onClose, onOpenStudy, title = "All DMD Clinical Trials" }: Props) {
  ...
  function SortIcon({ field }: { field: SortField }) {
    if (sortField !== field) return (
      <svg className="opacity-30 group-hover/th:opacity-60 transition-opacity" xmlns="http://www.w3.org/2000/svg" width="9" height="9" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
        <path d="M12 5v14M5 12l7-7 7 7"/>
      </svg>
    );
    return (
      <svg className="opacity-90" xmlns="http://www.w3.org/2000/svg" width="9" height="9" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
        {sortDir === "asc"
          ? <path d="M12 5v14M5 12l7-7 7 7"/>
              ...
              sorted.map((s) => {
                ...
                >
                  ...
                  <div className="flex items-center gap-1.5 min-w-0 group/title">
                    ...
                    <span className="text-[11px] text-[#0B3D52] truncate">{s.BriefTitle}</span>
                  </div>

                  <span className="text-[11px] text-[#6B8A96] truncate" title={s.LeadSponsorName}>{s.LeadSponsorName || "—"}</span>
